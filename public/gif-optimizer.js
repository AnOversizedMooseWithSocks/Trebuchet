(function installTrebuchetGifOptimizer() {
  'use strict';

  const scriptUrl = document.currentScript?.src || window.location.href;
  const codecUrl = new URL('./vendor/gif-codecs.mjs', scriptUrl).href;
  const workerScriptUrl = new URL('./gif-optimizer-worker.js?v=1', scriptUrl).href;
  ...
  async function optimizeAnimatedGif(file, options = {}) {
    ...
    if (!file || file.type !== 'image/gif') throw new Error('Animated GIF compression requires a GIF file');
    if (file.size <= 0 || file.size > SOURCE_MAX_BYTES) {
      throw new Error('Animated GIF source must be 10MB or smaller');
    }
    if (typeof Worker !== 'function' || typeof OffscreenCanvas !== 'function') {
      throw new Error('Animated GIF compression is unavailable in this runtime');
    }
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('Animated GIF compression timed out; use a shorter animation')), timeoutMs);
        worker.onmessage = (event) => {
          if (event.data?.id !== id || event.data?.progress != null) return;
          clearTimeout(timeout);
          if (event.data?.ok) resolve(event.data.result);
          else reject(new Error(event.data?.error || 'Animated GIF compression failed'));
        };
        worker.onerror = (event) => {
          clearTimeout(timeout);
          reject(new Error(event.message || 'Animated GIF compression worker failed'));
        };
        worker.postMessage({
          id,
          buffer: sourceBuffer,
      ...
      if (!result.compressed) return { ...result, file };
      const optimizedFile = new File([result.bytes], file.name || 'token-logo.gif', {
        type: 'image/gif',
        lastModified: Number(file.lastModified) || Date.now(),
      });
      return { ...result, bytes: undefined, file: optimizedFile };
    } finally {
      worker.terminate();
    }
  }

  window.TrebuchetGifOptimizer = Object.freeze({ optimizeAnimatedGif });
}());
